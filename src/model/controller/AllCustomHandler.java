    ...
    public static void onDragDropped(DragEvent event, Label lbl, StackPane imgGroup) {
        ...
        if (dragboard.hasContent(BasedEquipment.DATA_FORMAT)) {
            ...
            if (retrievedEquipment.getClass().getSimpleName().equals("Weapon")) {
                //++++++ add +++++++
                //magical
                if (((Weapon)retrievedEquipment).getDamageType() == DamageType.magical && (character.getType() == DamageType.block)){
                    Launcher.setEquippedWeapon(null);
                    Launcher.setAllEquipments(GenItemList.setUpItemList());
                    Launcher.refreshPane();
                    return;
                //physical
                } if ( ((Weapon)retrievedEquipment).getDamageType() == DamageType.block && (character.getType() == DamageType.magical)){
                    Launcher.setEquippedWeapon(null);
                    Launcher.setAllEquipments(GenItemList.setUpItemList());
                    Launcher.refreshPane();
                    return;
